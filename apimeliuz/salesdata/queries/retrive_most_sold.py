...
WHERE sd.product_id in (SELECT DISTINCT product_id FROM (SELECT DISTINCT product_id, SUM(sales_per_day) OVER (PARTITION BY product_id) as total_sales FROM salesdata_salesdata ORDER BY total_sales DESC) LIMIT 5)))
